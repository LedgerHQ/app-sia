...
def test_v2_sign_tx_refused(backend: BackendInterface, scenario_navigator: NavigateWithScenario):
...
def test_v2_sign_tx_accept(backend: BackendInterface, scenario_navigator: NavigateWithScenario):
    # Use the app interface instead of raw interface
    client = BoilerplateCommandSender(backend)
